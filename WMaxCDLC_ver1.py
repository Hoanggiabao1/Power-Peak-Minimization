...
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time, type, build_time, cal_time, filename="WMax_Ver1.csv"):
...
def solve_new(wcnf, var):
    ...
    try:
        result = subprocess.run(['wsl','./wmaxcdcl_static',wcnf_filename], capture_output=True, text=True, timeout=3600)
