...
def optimal(X,S,A,n,m,c,start_time):
    ...
    for idex in range(10):
        ...
        current_bestValue, station = result
        
        if current_bestValue < bestValue:
            bestValue = current_bestValue
            bestSolution = model
            print("new best value:",bestValue)
        for t in range(c):
    ...
    write_wcnf_with_h_prefix(clauses, soft_clauses, var, filename = "problem_maxHS.wcnf")
    model = solve_maxsat()
    if model is None:
        print("Time out")
...
def write_fancy_table_to_csv(ins, n, m, c, val, s_cons, h_cons, peak, status, time, filename="MaxHS_Binary_staircase_UB.csv"):
